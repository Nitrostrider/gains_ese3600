...
bool SetupIMU() {
    Serial.println("[IMU] Initializing I2C for IMU...");

    esp_err_t ret;  // Declare here for use throughout function

    // Check if I2C driver is already installed (by OLED)
    // Try a simple test transaction to see if bus is active
    i2c_cmd_handle_t test_cmd = i2c_cmd_link_create();
    i2c_master_start(test_cmd);
    i2c_master_stop(test_cmd);
    esp_err_t test_ret = i2c_master_cmd_begin(i2c_port, test_cmd, pdMS_TO_TICKS(10));
    i2c_cmd_link_delete(test_cmd);

    if (test_ret == ESP_OK || test_ret == ESP_ERR_TIMEOUT) {
        // I2C bus is already initialized (probably by OLED)
        Serial.println("[IMU] I2C already initialized (shared with OLED) - skipping config");
    } else {
        // I2C not initialized yet, set it up
        Serial.println("[IMU] I2C not initialized, configuring now...");
        ...
        if (ret != ESP_OK) {
            Serial.printf("[IMU] ERROR: I2C param config failed: %d\n", ret);
            return false;
        }
        Serial.println("[IMU] I2C param config OK");

        ret = i2c_driver_install(i2c_port, conf.mode, 0, 0, 0);
        if (ret != ESP_OK) {
            Serial.printf("[IMU] ERROR: I2C driver install failed: %d\n", ret);
            return false;
        }
        Serial.println("[IMU] I2C driver installed");
    }
    ...
    uint8_t who_am_i = 0;
    Serial.printf("[IMU] Reading WHO_AM_I from address 0x%02X...\n", kIMU_Address);
    ret = i2c_read_bytes(kIMU_Address, ICM20600_Regs::WHO_AM_I, &who_am_i, 1);
    if (ret != ESP_OK) {
        Serial.printf("[IMU] ERROR: Failed to read WHO_AM_I at address 0x%02X, error=%d\n", kIMU_Address, ret);
        Serial.println("[IMU] Check: 1) IMU connected? 2) Correct I2C address? 3) I2C pins correct?");
        return false;
    }
    Serial.printf("[IMU] WHO_AM_I: 0x%02X (expected 0x11 for ICM-20600)\n", who_am_i);

    if (who_am_i != 0x11) {
        Serial.printf("[IMU] WARNING: Unexpected WHO_AM_I value 0x%02X! Continuing anyway...\n", who_am_i);
    }
