...
static esp_err_t oled_init_hw(void) {
    ...
    static constexpr uint8_t cmd_clk[]      = {0xD5, 0x80};
    static constexpr uint8_t cmd_mux[]      = {0xA8, 0x3F};
    static constexpr uint8_t cmd_offset[]   = {0xD3, 0x00};
    static constexpr uint8_t cmd_chgpump[]  = {0x8D, 0x14};
    static constexpr uint8_t cmd_addrmode[] = {0x20, 0x00};
    static constexpr uint8_t cmd_compins[]  = {0xDA, 0x12};
    static constexpr uint8_t cmd_contrast[] = {0x81, 0xCF};
    static constexpr uint8_t cmd_precharge[] = {0xD9, 0xF1};
    static constexpr uint8_t cmd_vcom[]      = {0xDB, 0x40};

    err |= oled_cmd_list(cmd_clk,      sizeof(cmd_clk));
    err |= oled_cmd_list(cmd_mux,      sizeof(cmd_mux));
    err |= oled_cmd_list(cmd_offset,   sizeof(cmd_offset));
    err |= oled_cmd(0x40); // Start line = 0
    err |= oled_cmd_list(cmd_chgpump,  sizeof(cmd_chgpump));
    err |= oled_cmd_list(cmd_addrmode, sizeof(cmd_addrmode));
    err |= oled_cmd(0xA1); // Segment remap
    err |= oled_cmd(0xC8); // COM scan direction remap
    err |= oled_cmd_list(cmd_compins,  sizeof(cmd_compins));
    err |= oled_cmd_list(cmd_contrast, sizeof(cmd_contrast));
    err |= oled_cmd_list(cmd_precharge,sizeof(cmd_precharge));
    err |= oled_cmd_list(cmd_vcom,     sizeof(cmd_vcom));

    err |= oled_cmd(0xA4); // Resume RAM
